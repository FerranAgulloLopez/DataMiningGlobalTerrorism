...
We think it might be interesting to know whether the claimed mode affects or helps capture a perpetrator.
...
And we can see better that "Letter" and "Note left at scene" are the next two claimed modes with the biggest number of captured perpetrators.
...
The same type of study as in "weaptype1_txt / nkill" section we repeate with "weaptype1_txt / nwound".
...
#### provstate (top 3 provstate) / gname
...
plot_ly(aux, labels = ~aux, values = ~Freq, type = 'pie') %>%
  layout(title = 'Pie Chart of different gname in California')
```
...
View(b)
plot_ly(b, labels = ~Rate, values = ~Freq, type = 'pie')
```
